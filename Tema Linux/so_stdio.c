...
SO_FILE *so_popen(const char *command, const char *type)
{
    ...
    else if(pid==0)
    {
        ...
        execlp("bash","sh","-c",command,NULL);//executam comanda in bash
        exit(1);
    }
    else{
        int filed=0;//variabila pentru a retine file descriptorul corespunzator capatului de pipe
        if(strcmp(command,"r")==0)
        {
            close(fd[1]);//daca in procesul parinte citim, putem inchide capatul pentru scriere
            filed=fd[0];//retinem fd-ul corespunzator operatiei
        }
        else if(strcmp(command,"w")==0)
        {
            close(fd[0]);//daca in procesul parinte scriem, putem inchide capatul de citire
            filed=fd[1];//retinem fd-ul corespunzator operatiei
        }
         f->child=pid;
         f->fd=filed;
    }
...
int so_pclose(SO_FILE *stream)
{
   ...
   if(ret<0){
    ...
    int status;
    ret=waitpid(stream->child,&status,0);
    if(ret<0){
        stream->_error=1;
        return SO_EOF;
    }
...
int main()
{
    ...
    int ret = fwrite(tmp, 1,sizeof(tmp)-1, f);
    printf("%d",ret);
    printf("%d",ferror(f));
    fclose(f);*/
